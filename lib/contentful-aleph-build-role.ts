...
export interface IContentfulAlephBuildRoleProps extends RoleProps {
  readonly stages: string[]
  readonly artifactBucket: Bucket
}

export class ContentfulAlephBuildRole extends Role {
  constructor(scope: Construct, id: string, props: IContentfulAlephBuildRoleProps) {
    super(scope, id, props)

    const serviceStackPrefix = scope.node.tryGetContext('serviceStackName') || 'contentful-aleph'
    ...
    this.addToPolicy(
      new PolicyStatement({
        resources: [
          Fn.sub('arn:aws:ssm:${AWS::Region}:${AWS::AccountId}:parameter/all/contentful-aleph/*'),
    ...
    this.addToPolicy(
      new PolicyStatement({
        resources: [Fn.sub('arn:aws:secretsmanager:${AWS::Region}:${AWS::AccountId}:secret:/all/contentful/*')],
        actions: [
...
export default ContentfulAlephBuildRole
